...
/*
  ...
  struct TreeNode { int item; struct TreeNode* next; };

  struct TreeNode* build_list() {
    struct TreeNode** pool;
    int n = read_number_of_nodes_needed();
    if (n <= 0) return 0;
    pool = (struct TreeNode**)(independent_calloc(n, sizeof(struct TreeNode), 0);
    if (pool == 0) die();
    // organize into a linked list...
    struct TreeNode* first = pool[0];
    for (i = 0; i < n-1; ++i)
